module FastJsonapi
  class Relationship
    attr_reader :key, :name, :id_method_name, :type_method_name, :record_type, :object_method_name, :object_block, :serializer, :relationship_type, :cached, :polymorphic, :conditional_proc, :transform_method, :links, :lazy_load_data
    ...
    def initialize(
      ...
      @type_method_name = type_method_name
    ...
    def ids_hash_from_record_and_relationship(record, params = {})
      return ids_hash(
        fetch_id(record, params), fetch_type(record, params)
      ) unless polymorphic
    ...
    def fetch_type(record, params)
      if object_block.present?
        object = object_block.call(record, params)
        return object.map { |item| item.public_send(type_method_name) } if object.respond_to? :map
        return object.try(type_method_name)
      end
      begin
